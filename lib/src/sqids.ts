type SqidsInteger<Mode extends "number" | "bigint"> = Mode extends "bigint" ? bigint : number;
...
export default class Sqids<Mode extends "number" | "bigint" = "number"> {
  ...
  constructor(options?: SqidsOptions<Mode>) {
    ...
    const alphabet =
      options?.alphabet === undefined ? internalDefaultOptions.alphabet : options.alphabet;
    const minLength =
      options?.minLength === undefined ? internalDefaultOptions.minLength : options.minLength;
    const blocklist =
      options?.blocklist === undefined ? internalDefaultOptions.blocklist : options.blocklist;
    const mode = options?.mode === undefined ? internalDefaultOptions.mode : options.mode;
    ...
    if (alphabet.length < minAlphabetLength) {
      throw new SqidsError(`Alphabet length must be at least ${minAlphabetLength}`);
    }
    ...
    if (!Number.isInteger(minLength) || minLength < 0 || minLength > minLengthLimit) {
      throw new SqidsError(`Minimum length has to be between 0 and ${minLengthLimit}`);
    }
  ...
  encode(numbers: readonly SqidsInteger<Mode>[]): string {
    ...
    const normalizedNumbers = numbers.map((number) => {
      if (this.mode === "bigint") {
        if (typeof number !== "bigint" || number < 0n || number > this.maxValue()) {
          throw new SqidsError(`Encoding supports bigints between 0n and ${this.maxValue()}n`);
        }
      ...
      if (typeof number !== "number" || !Number.isSafeInteger(number) || number < 0) {
        throw new SqidsError(`Encoding supports numbers between 0 and ${this.maxValue()}`);
      }
  ...
  decode(id: string): SqidsInteger<Mode>[] {
    ...
    while (slicedId.length > 0) {
      ...
      if (chunks.length > 0) {
        ...
        if (!chunk) {
          return (this.mode === "bigint" ? ret : ret.map(Number)) as SqidsInteger<Mode>[];
        }
  ...
  private shuffle(alphabet: string): string {
    ...
    for (let i = 0, j = chars.length - 1; j > 0; i++, j--) {
      ...
      const r = (i * j + left.charCodeAt(0) + last.charCodeAt(0)) % chars.length;
      const right = chars[r];
      if (right === undefined) {
        throw new SqidsError("Reached an invalid alphabet index");
      }
  ...
  private toNumber(id: string, alphabet: string): bigint {
    ...
    return id.split("").reduce((a, v) => a * alphabetLength + BigInt(chars.indexOf(v)), 0n);
  }

...
describe("options", () => {
  it.each([null, [], "invalid", 1])("rejects non-object options: %j", (options) => {
    expect(() => new Sqids(options as never)).toThrow("Options must be an object");
  });

  it("rejects explicit null option values", () => {
    expect(() => new Sqids({ alphabet: null as never })).toThrow("Alphabet must be a string");
  ...
  it("rejects a non-string alphabet", () => {
    expect(() => new Sqids({ alphabet: 123 as never })).toThrow("Alphabet must be a string");
  });
...
describe("bigint", () => {
  ...
  it("uses the same encoding as number mode for safe integers", () => {
    ...
    expect(bigintSqids.encode([1n, 2n, 3n])).toBe(numberSqids.encode([1, 2, 3]));
  });
  ...
  it("preserves mode-specific return types", () => {
    ...
    expectTypeOf(new Sqids({ mode: "number" }).decode("")).toEqualTypeOf<number[]>();
    expectTypeOf(new Sqids({ mode: "bigint" }).decode("")).toEqualTypeOf<bigint[]>();
  });
...
describe("encoding", () => {
  ...
  it.each([null, {}, "123"])("rejects non-array input: %j", (numbers) => {
    expect(() => new Sqids().encode(numbers as never)).toThrow("Numbers must be an array");
  });
  ...
  it.each([null, 123, []])("rejects non-string ID: %j", (id) => {
    expect(() => new Sqids().decode(id as never)).toThrow("ID must be a string");
  });
...
describe("min length", () => {
  it.each([-1, 256, 1.5, Number.NaN, Number.POSITIVE_INFINITY])(
    ...
    (minLength) => {
      expect(() => new Sqids({ minLength })).toThrow("Minimum length has to be between 0 and 255");
    },
  );
